...
import pandas as pd
import numpy as np
from dataloader import cpi_data, create_bar_chart, create_line_chart
...
cpi = cpi_data()
level1 = cpi['Index'].unique()
level1 = pd.Series(level1)
level1 = level1[level1.str.match(r'^1(\.\d+)?$')]
#select_level = st.radio("Select Index",level1, horizontal=True, key = 'fig1')
level1_df = cpi[cpi['Index'].isin(level1)][['Date', 'Name', 'Values']]
level1_df = level1_df.pivot_table(index='Date', columns='Name', values='Values').reset_index()

pct_mom = level1_df.copy()
pct_yoy = level1_df.copy()
...
with tab1:
    st.plotly_chart(fig2, key='top_yoy')
with tab2:
    st.plotly_chart(fig1, key='top_mom')
    

st.divider()
#-----------------------------------------------------------------------------------------------------------------------------
# Part 2
l1_table = pd.melt(level1_df,id_vars='Date',var_name='Name',value_name='Values')
...
with tab1:
    st.plotly_chart(fig1, key='l1_level')
    st.plotly_chart(fig2, key='l1_mom')
    st.plotly_chart(fig3, key='l1_yoy')
with tab2:
   st.dataframe(l1_df,width='stretch')
   st.dataframe(pct_mom,width='stretch')
   st.dataframe(pct_yoy,width='stretch')
with tab3:
    df_diff = level1_df.copy()
    ...
    if selection:
        ...
        st.plotly_chart(fig, key='l1_dist')

# #--------------------------------------------------------------------------------------------------
parent_index = cpi['Parent Index'].unique()
parent_index = pd.Series(parent_index)
parent_index = parent_index[parent_index.str.match(r'^1(\.\d+)?$')]
level2 = cpi[cpi['Parent Index'].astype(str).isin(parent_index)]
parent_map = dict(zip(level2['Parent Index'].astype(str), level2['Parent Name']))
selected_parent_index = st.selectbox("Select Parent Category", options=list(parent_map.keys()), format_func=lambda x: parent_map[x], key='l2_parent')
level2_all = level2[level2['Parent Index'].astype(str) == selected_parent_index][['Date', 'Name', 'Values']]
all_l2_names = sorted(level2_all['Name'].unique().tolist())
selected_l2_names = st.multiselect("Select series to display", all_l2_names, default=all_l2_names[:6], key='l2_series')
level2_chart = level2_all[level2_all['Name'].isin(selected_l2_names)] if selected_l2_names else level2_all
...
with tab3:
    st.plotly_chart(fig1, key='l2_level')
    st.plotly_chart(fig2, key='l2_mom')
    st.plotly_chart(fig3, key='l2_yoy')
with tab4:
...
# Filter for '1.x.x' format in 'Parent Index'
level3 = cpi[cpi['Parent Index'].notna() & cpi['Parent Index'].str.match(r'^1\.\d+\.\d+$')]
...
selected_parent_index = st.selectbox("Select Parent Category", options=list(parent_map.keys()), format_func=lambda x: parent_map[x], key='l3_parent')
level3_chart = cpi[cpi['Parent Index'].astype(str) == selected_parent_index][['Date', 'Name', 'Values']]
...
tab3, tab4 = st.tabs(["📈 Chart", "🗃 Data"])
with tab3:
    st.plotly_chart(fig1, key='l3_level')
    st.plotly_chart(fig2, key='l3_mom')
    st.plotly_chart(fig3, key='l3_yoy')
with tab4:
...
# Filter for '1.x.x' format in 'Parent Index'
level4 = cpi[cpi['Parent Index'].notna() & cpi['Parent Index'].str.match(r'^1\.\d+\.\d+\.\d+$')]
# Extract unique Parent Index values
...
selected_parent_index = st.selectbox("Select Parent Category", options=list(parent_map.keys()), format_func=lambda x: parent_map[x], key='l4_parent')
level4_chart = cpi[cpi['Parent Index'].astype(str) == selected_parent_index][['Date', 'Name', 'Values']]
...
tab3, tab4 = st.tabs(["📈 Chart", "🗃 Data"])
with tab3:
    st.plotly_chart(fig1, key='l4_level')
    st.plotly_chart(fig2, key='l4_mom')
    st.plotly_chart(fig3, key='l4_yoy')
with tab4:
...
if selection:
    ...
    st.dataframe(dis_table,width='stretch')
    st.plotly_chart(fig, key='l1_dist2')
