...
from dataloader import business_inv_data, line_chart

# ── Data ────────────────────────────────────────────────────────────────────
biz = business_inv_data()

# ── Section 1: Top-level selector (Sales / Inventory / Ratio) ───────────────
depth1_levels = sorted(
    [lvl for lvl in biz['Index Level'].unique() if '.' not in lvl],
    key=lambda x: int(x)
)
depth1_names = {lvl: biz[biz['Index Level'] == lvl]['Name'].iloc[0] for lvl in depth1_levels}
depth1_units = {lvl: biz[biz['Index Level'] == lvl]['Unit'].iloc[0] for lvl in depth1_levels}
...
unit_label = depth1_units.get(top_level, '')

# All series under selected top group
level_df = biz[
    biz['Index Level'].str.startswith(top_level + '.') |
    (biz['Index Level'] == top_level)
][['Date', 'Name', 'Values']]
...
bar_height = max(300, len(snap_mom) * 30)
...
# ── Section 2: Sub-sector time series ───────────────────────────────────────
st.write("**Select sub-sector to plot over time**")

sub_df = biz[biz['Parent Index'] == top_level]
if sub_df.empty:
    sub_df = biz[biz['Index Level'] == top_level]

all_names = sorted(sub_df['Name'].unique().tolist())

if len(all_names) > 6:
    selected = st.multiselect('Select series:', all_names, default=all_names)
else:
    selected = []
...
if not selected:
    ...
    sel_df = sub_df[sub_df['Name'].isin(selected)][['Date', 'Name', 'Values']]
...
selection = st.radio("Select Series", [c for c in df_diff.columns], horizontal=True)
