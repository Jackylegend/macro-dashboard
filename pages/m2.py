...
import pandas as pd
from dataloader import m2_data
...
index_df = table[display_col]

indices = table['Name'].unique()
type_ = st.radio("Data Type", indices, horizontal=True, key='part1')

index_df = index_df[index_df['Name'] == type_]
index_df = index_df.pivot_table(index='Date', columns='Name', values='Values').reset_index()
index_df['Pct_Change'] = index_df.iloc[:, 1].pct_change()

fig1 = px.line(index_df, x='Date', y=index_df[index_df.columns[1]])
fig1.update_layout(xaxis=dict(linecolor='black', linewidth=1,showgrid=True),
                    yaxis=dict(linecolor='black', linewidth=2),
                    yaxis_range=[(index_df[index_df.columns[1]].min()) * 0.9, (index_df[index_df.columns[1]].max()) * 1.2])
fig1.update_traces(hoverlabel=dict(font_size=18))

fig2 = px.bar(index_df, x='Date', y='Pct_Change')
fig2.update_layout(xaxis=dict(linecolor='black', linewidth=1,autorange=True,showgrid=True,constrain='domain'),
                    yaxis=dict(linecolor='black', linewidth=2,constrain='domain'),autosize=True,
                    yaxis_range=[(index_df['Pct_Change'].min()) * 1.1, (index_df['Pct_Change'].max()) * 1.2],
                    hovermode='closest')
fig2.update_traces(hoverlabel=dict(font_size=18))
...
selection = st.radio("Data Type", select_columns, horizontal=True,key='part2')
if selection:
