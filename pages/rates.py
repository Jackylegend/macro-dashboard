import streamlit as st
import pandas as pd
from dataloader import yield_data
import numpy as np
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import plotly.express as px

type_, freq, rates = yield_data()
data_freq = freq
data_type = type_
ir = rates
# loading dataset from loader:
df_order_col = ['Fed Fund Rate','1-month','3-month','6-month','1-year','2-year','3-year','5-year','7-year','10-year','20-year','30-year']
filtered_freq = data_freq
filtered_type = data_type
tip_order_col = ['5-year','7-year','10-year','20-year','30-year']
st_order_col = ('Date','Fed Fund Rate','1-month','3-month','6-month','1-year','2-year','3-year','5-year','7-year','10-year','20-year','30-year','Bank prime loan','Discount window primary credit')

col1, col2 = st.columns(2)
with col1:
    type_ = st.radio("Data Type", filtered_type, horizontal=True)
with col2:
    freq = st.radio('Select Frequency:', filtered_freq, horizontal=True)

df = ir[(ir['Frequency'] == freq) & (ir['Type'] == type_)][['Date', 'Name', 'Values']]
column_names = [c for c in st_order_col if c in df['Name'].unique()]
st.write("Select Level 1 Index:")
cols = st.columns(len(column_names))
selected_names = []
for i, name in enumerate(column_names):
    with cols[i]:
        if st.checkbox(name, value=(name == 'Fed Fund Rate')):  # Fed Fund Rate checked by default
            selected_names.append(name)
yields_chart = df[df['Name'].isin(selected_names)]
yields_df = yields_chart.pivot_table(index='Date', columns='Name', values='Values').round(2)
yields_df = yields_df[[c for c in st_order_col if c in yields_df.columns]]
if 'Fed Fund Rate' in selected_names:
    if len(selected_names) == 1:  # Only Fed Fund Rate is selected
        yields_df = yields_df[['Fed Fund Rate']]  # Display only the Fed Fund Rate
    else:  # Fed Fund Rate is selected with other items
        yields_df['Fed Fund Rate'] = yields_df['Fed Fund Rate'].ffill()
        value_cols = yields_df.columns.difference(['Date'])
        mask = yields_df['Fed Fund Rate'].notna() & yields_df[value_cols].drop(columns=['Fed Fund Rate']).isna().all(axis=1)
        yields_df = yields_df[~mask]

diff_df = pd.DataFrame(columns=['Date'])
diff_chart = pd.DataFrame(columns=['Date', 'Name', 'Values'])
if not yields_df.empty:
    diff_df = yields_df.copy().reset_index()
    diff_df.iloc[:, 1:] = diff_df.iloc[:, 1:].diff().round(2)
    diff_chart = diff_df.melt(id_vars='Date', var_name='Name', value_name='Values')

diff_stats= df.pivot_table(index='Date', columns='Name', values='Values')
diff_stats = diff_stats[[c for c in st_order_col if c in diff_stats.columns]]
if 'Fed Fund Rate' in diff_stats.columns:
    diff_stats['Fed Fund Rate'] = diff_stats['Fed Fund Rate'].ffill()
    value_cols = diff_stats.columns.difference(['Date'])
    mask = diff_stats['Fed Fund Rate'].notna() & diff_stats[value_cols].drop(columns=['Fed Fund Rate']).isna().all(axis=1)
    diff_stats = diff_stats[~mask]
    for column in diff_stats.columns:
        if column != 'Date':
            diff_stats[column] = diff_stats[column].diff()


#-----------------------------------------------------------------------------------
#Plotly chart setup
fig1 = px.bar(diff_chart, x='Date', y='Values', color='Name')
fig1.add_hline(y=0, line_dash="solid", line_color="black")
y_min, y_max = diff_chart["Values"].min(), diff_chart["Values"].max()
tick_interval = (y_max - y_min) / 10  # Adjust 10 ticks for better readability
fig1.update_layout(barmode='group',xaxis=dict(linecolor='black', tickfont=dict(color='black'),title='',linewidth=1,autorange=True,showgrid=True,nticks=10),
                        yaxis=dict(tickformat=".2f",linecolor='black', tickfont=dict(color='black'),title='',linewidth=2),autosize=True,height=600)   
fig2 = px.line(yields_chart, x='Date', y='Values', color='Name')
fig2.add_hline(y=0, line_dash="solid", line_color="black")
fig2.update_layout(xaxis=dict(linecolor='black', tickfont=dict(color='black'),title='',linewidth=1,autorange=True,showgrid=True,nticks=10),
                        yaxis=dict(tickformat=".2f",linecolor='black', tickfont=dict(color='black'),title='',linewidth=2),autosize=True,height=600)

#----------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Summary Statistics def function
def summary_statistics(df,key='header'):
    df_diff = df.copy()
    #df_diff['Values'] = pd.to_numeric(df_diff['Values'], errors='coerce')
    # df_diff = df_diff.pivot_table(index='Date', columns='Name', values='Values')

    mean = df_diff.mean(numeric_only=True).to_frame("Mean")
    stand_error = df_diff.sem(numeric_only=True).to_frame("Standard Error")
    median = df_diff.median(numeric_only=True).to_frame("Median")
    mode_result = df_diff.mode(numeric_only=True)
    mode = (mode_result.iloc[0] if not mode_result.empty else df_diff.mean(numeric_only=True)).to_frame("Mode")
    std = df_diff.std(numeric_only=True).to_frame("Standard Deviation")
    s_var = df_diff.var(numeric_only=True).to_frame("Sample Variance")
    kurt = df_diff.kurt(numeric_only=True).to_frame("Kurtosis")
    skew = df_diff.skew(numeric_only=True).to_frame("Skew")
    range = df_diff.max(numeric_only=True) - df_diff.min(numeric_only=True)
    range = range.to_frame("Range")
    min = df_diff.min(numeric_only=True).to_frame("Min")
    max = df_diff.max(numeric_only=True).to_frame("Max")
    sum = df_diff.sum(numeric_only=True).to_frame("Sum")
    count = df_diff.count(numeric_only=True).to_frame("Count") 

    stats_table = pd.concat([mean, stand_error, median, mode, std, s_var, kurt, skew, range, min, max, sum, count], axis=1).round(2)
    stats_table = stats_table.T.reset_index().rename(columns={'index': 'Descriptive Statistics'})
    std_table = stats_table.copy()
    value_columns = [col for col in std_table.columns if col != 'Descriptive Statistics']
    std_bounce =[{'Standard Deviation Bounds': '1 STD Lower Bound'},
                {'Standard Deviation Bounds': '1 STD Upper Bound'},
                {'Standard Deviation Bounds': '2 STD Lower Bound'},
                {'Standard Deviation Bounds': '2 STD Upper Bound'},
                {'Standard Deviation Bounds': '3 STD Lower Bound'},
                {'Standard Deviation Bounds': '3 STD Upper Bound'},
                {'Standard Deviation Bounds': 'Actual Count'},
                {'Standard Deviation Bounds': 'Actual Count'},
                {'Standard Deviation Bounds': 'Actual Count'},
                {'Standard Deviation Bounds': 'Actual % Count'},
                {'Standard Deviation Bounds': 'Actual % Count'},
                {'Standard Deviation Bounds': 'Actual % Count'},
                {'Standard Deviation Bounds': 'Normal % Count'},
                {'Standard Deviation Bounds': 'Normal % Count'},
                {'Standard Deviation Bounds': 'Normal % Count'}]
    posi_prob_adj =[{'Positive Probability Adjusted Return': 'Mean Change'},
                {'Positive Probability Adjusted Return': 'Count'},
                {'Positive Probability Adjusted Return': 'Frequency %'},
                {'Positive Probability Adjusted Return': 'Prob Adj Change'}]
    nega_prob_adj =[{'Negative Probability Adjusted Return': 'Mean Change'},
                {'Negative Probability Adjusted Return': 'Count'},
                {'Negative Probability Adjusted Return': 'Frequency %'},
                {'Negative Probability Adjusted Return': 'Prob Adj Change'}]
    zero_prob_adj =[{'Zero Return': 'Mean Change'},
                {'Zero Return': 'Count'},
                {'Zero Return': 'Frequency %'},
                {'Zero Return': 'Prob Adj Change'}]
    percentiles = {'Percentiles' : ['1%', '2%', '3%', '4%', '5%','10%', '25%', '50%', '75%', '90%', '95%', '96%', '97%', '98%', '99%']}
    percentile_values = [1, 2, 3, 4, 5, 10, 25, 50, 75, 90, 95, 96, 97, 98, 99]

    for col in value_columns:
        mean_row = std_table.loc[std_table['Descriptive Statistics'] == 'Mean', col].values
        std_row = std_table.loc[std_table['Descriptive Statistics'] == 'Standard Deviation', col].values
        total_count = std_table.loc[std_table['Descriptive Statistics'] == 'Count', col].values
        count_value = df_diff[col].values
        std_bounce[0][col] = round(float(mean_row - (1 * std_row)), 2)
        std_bounce[1][col] = round(float(mean_row + (1 * std_row)), 2)
        std_bounce[2][col] = round(float(mean_row - (2 * std_row)), 2)
        std_bounce[3][col] = round(float(mean_row + (2 * std_row)), 2)
        std_bounce[4][col] = round(float(mean_row - (3 * std_row)), 2)
        std_bounce[5][col] = round(float(mean_row + (3 * std_row)), 2)
        std_bounce[6][col] = np.sum((count_value >= std_bounce[0][col]) & (count_value <= std_bounce[1][col]))
        std_bounce[7][col] = np.sum((count_value >= std_bounce[2][col]) & (count_value <= std_bounce[3][col]))
        std_bounce[8][col] = np.sum((count_value >= std_bounce[4][col]) & (count_value <= std_bounce[5][col]))
        std_bounce[9][col] = "{:.2%}".format(float(std_bounce[6][col] / total_count))
        std_bounce[10][col] = "{:.2%}".format(float(std_bounce[7][col] / total_count))
        std_bounce[11][col] = "{:.2%}".format(float(std_bounce[8][col] / total_count))
        std_bounce[12][col] = "{:.2%}".format(0.6827)
        std_bounce[13][col] = "{:.2%}".format(0.9545)
        std_bounce[14][col] = "{:.2%}".format(0.9973)
        std1_bound = pd.DataFrame([std_bounce[0],std_bounce[1],std_bounce[6],std_bounce[9],std_bounce[12]])
        std2_bound = pd.DataFrame([std_bounce[2],std_bounce[3],std_bounce[7],std_bounce[10],std_bounce[13]])
        std3_bound = pd.DataFrame([std_bounce[4],std_bounce[5],std_bounce[8],std_bounce[11],std_bounce[14]])

    # Positive prob adjusted return
        posi_mean = np.mean((count_value > 0))
        posi_count = np.sum((count_value > 0))
        freq_pct = float(posi_count / total_count)
        posi_prob_adj[0][col] = round(posi_mean, 2)
        posi_prob_adj[1][col] = posi_count
        posi_prob_adj[2][col] = "{:.2%}".format(freq_pct)
        posi_prob_adj[3][col] = round(posi_prob_adj[0][col] * freq_pct, 2)

    # Negative prob adjusted return
        naga_mean = np.mean((count_value < 0))
        nega_count = np.sum((count_value < 0))
        freq_pct = float(nega_count / total_count)
        nega_prob_adj[0][col] = round(naga_mean, 2)
        nega_prob_adj[1][col] = nega_count
        nega_prob_adj[2][col] = "{:.2%}".format(freq_pct)
        nega_prob_adj[3][col] = round(nega_prob_adj[0][col] * freq_pct, 2)

    # Zero return
        zero_mean = np.mean((count_value == 0))
        zero_count = np.sum((count_value == 0))
        freq_pct = float(zero_count / total_count)
        zero_prob_adj[0][col] = round(zero_mean, 2)
        zero_prob_adj[1][col] = zero_count
        zero_prob_adj[2][col] = "{:.2%}".format(freq_pct)
        zero_prob_adj[3][col] = round(zero_prob_adj[0][col] * freq_pct, 2)
        positive_adj = pd.DataFrame([posi_prob_adj[0],posi_prob_adj[1],posi_prob_adj[2],posi_prob_adj[3]])
        negative_adj = pd.DataFrame([nega_prob_adj[0],nega_prob_adj[1],nega_prob_adj[2],nega_prob_adj[3]])
        zero_adj = pd.DataFrame([zero_prob_adj[0],zero_prob_adj[1],zero_prob_adj[2],zero_prob_adj[3]])

    # Percentiles
    percentiles_df = pd.DataFrame(percentiles)
    for col in value_columns:
        percentile_values_col = np.nanpercentile(df_diff[col], percentile_values)
        percentiles_df[col] = percentile_values_col

    # Probability Distribution Table
    diff_table = df_diff.copy()
    select_columns = [col for col in diff_table.columns if col != 'Date']
    selection = st.radio("Data Type", select_columns, horizontal=True,key=key)
    if selection:
        distribution = diff_table[selection].dropna().values
        dis_mean = np.mean(distribution)
        dis_std = np.std(distribution)
        std_interval = [-3,-2.5,-2,-1.75,-1.5,-1.25,-1,-0.8,-0.6,-0.4,-0.2,0,0.2,0.4,0.6,0.8,1,1.25,1.5,1.75,2,2.5,3, np.inf]
        bin_val = [dis_mean + value * dis_std if value is not np.inf else np.inf for value in std_interval]
        bin_label = [round(dis_mean + value * dis_std, 2) if value != np.inf else 'More' for value in std_interval]
        dis_table = pd.DataFrame({'Interval': std_interval,'Bin Value': bin_val,'Bin Label': bin_label})
        try:
            bins = [-np.inf] + dis_table['Bin Value'].tolist()
            cat_result = pd.cut(distribution, bins=bins, labels=False, right=False, include_lowest=True)
            vc = pd.Series(cat_result).value_counts()
            vc.index = vc.index.astype(int)
            value_counts = vc.reindex(np.arange(24), fill_value=0)
        except ValueError:
            value_counts = pd.Series(0, index=np.arange(24))
            value_counts[11] = len(distribution)
        dis_table['Count'] = value_counts
        range_label = [f"Less than {dis_table['Bin Label'][0]}",f"{dis_table['Bin Label'][0]} to {dis_table['Bin Label'][1]}",
                        f"{dis_table['Bin Label'][1]} to {dis_table['Bin Label'][2]}",
                        f"{dis_table['Bin Label'][2]} to {dis_table['Bin Label'][3]}",
                        f"{dis_table['Bin Label'][3]} to {dis_table['Bin Label'][4]}",
                        f"{dis_table['Bin Label'][4]} to {dis_table['Bin Label'][5]}",
                        f"{dis_table['Bin Label'][5]} to {dis_table['Bin Label'][6]}",
                        f"{dis_table['Bin Label'][6]} to {dis_table['Bin Label'][7]}",
                        f"{dis_table['Bin Label'][7]} to {dis_table['Bin Label'][8]}",
                        f"{dis_table['Bin Label'][8]} to {dis_table['Bin Label'][9]}",
                        f"{dis_table['Bin Label'][9]} to {dis_table['Bin Label'][10]}",
                        f"{dis_table['Bin Label'][10]} to {dis_table['Bin Label'][11]}",
                        f"{dis_table['Bin Label'][11]} to {dis_table['Bin Label'][12]}",
                        f"{dis_table['Bin Label'][12]} to {dis_table['Bin Label'][13]}",
                        f"{dis_table['Bin Label'][13]} to {dis_table['Bin Label'][14]}",
                        f"{dis_table['Bin Label'][14]} to {dis_table['Bin Label'][15]}",
                        f"{dis_table['Bin Label'][15]} to {dis_table['Bin Label'][16]}",
                        f"{dis_table['Bin Label'][16]} to {dis_table['Bin Label'][17]}",
                        f"{dis_table['Bin Label'][17]} to {dis_table['Bin Label'][18]}",
                        f"{dis_table['Bin Label'][18]} to {dis_table['Bin Label'][19]}",
                        f"{dis_table['Bin Label'][19]} to {dis_table['Bin Label'][20]}",
                        f"{dis_table['Bin Label'][20]} to {dis_table['Bin Label'][21]}",
                        f"{dis_table['Bin Label'][21]} to {dis_table['Bin Label'][22]}",
                        f"More than {dis_table['Bin Label'][22]}"]
        count_prob = dis_table['Count'] / pd.Series(distribution).count()
        cum_prob = count_prob.cumsum()
        dis_table['Range'] = range_label
        dis_table['Probability'] = count_prob.apply(lambda x: f"{x*100:.2f}%")
        dis_table['Cum. Probability'] = cum_prob.apply(lambda x: f"{x*100:.2f}%")

        chart_table = dis_table.copy()
        chart_table['Probability'] = count_prob * 100
        adjusted_y = [y / 100 for y in chart_table['Probability']]

        #graph
        tittle_name = f'{selection} Change Histogram'
        fig = px.bar(chart_table,x='Range',y=adjusted_y,title=tittle_name,
                        labels={'y': 'Probability (%)', 'Range': 'Range'},text='Probability')
        fig.update_traces(texttemplate='%{text:.2f}%',textposition='outside')
        fig.update_layout(yaxis=dict(tickformat=".2%"),xaxis=dict(tickangle=-45))

    st.plotly_chart(fig)
    st.dataframe(stats_table,hide_index=True,height=500)
    st.dataframe(std1_bound,hide_index=True,height=230)
    st.dataframe(std2_bound,hide_index=True,height=230)
    st.dataframe(std3_bound,hide_index=True,height=230)
    st.dataframe(positive_adj,hide_index=True,height=200)
    st.dataframe(negative_adj,hide_index=True,height=200)
    st.dataframe(zero_adj,hide_index=True,height=200)
    st.dataframe(percentiles_df.round(2),hide_index=True,height=580)
    st.dataframe(dis_table.round(2),width='stretch',hide_index=True,height=900)
#-----------------------------------------------------------------------------------------------------------------------------------------------

tab1, tab2, tab3,tab4 = st.tabs(["📈 Line Chart", "📈 Bar Chart","🗃 Data Table","🗃 Summary Statistics"])
with tab1:
    st.plotly_chart(fig2)
with tab2:
    st.plotly_chart(fig1)
with tab3:
    st.dataframe(yields_df,width='stretch')
    st.dataframe(diff_df,width='stretch')
with tab4:
    summary_statistics(diff_stats,key='header')

st.divider()
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
tip_df = ir[(ir['Frequency'] == freq) & (ir['Type'] == 'Tips')][['Date', 'Name', 'Values']]
tip_df= tip_df.pivot_table(index='Date', columns='Name', values='Values').round(2)
tip_df = tip_df[[c for c in st_order_col if c in tip_df.columns]]
tip_df = tip_df.dropna()
tip_df = tip_df.sort_values(by='Date', ascending=False)

spread_df = ir[(ir['Frequency'] == freq) & (ir['Type'] == 'Yield')][['Date', 'Name', 'Values']]
spread_df = spread_df.pivot_table(index='Date', columns='Name', values='Values').round(2)
spread_df = spread_df[[c for c in st_order_col if c in spread_df.columns]]
spread_df['Fed Fund Rate'] = spread_df['Fed Fund Rate'].ffill()
value_cols = spread_df.columns.difference(['Date'])
mask = spread_df['Fed Fund Rate'].notna() & spread_df[value_cols].drop(columns=['Fed Fund Rate']).isna().all(axis=1)
spread_df = spread_df[~mask]
spread_df = spread_df.dropna()
spread_df = spread_df.sort_values(by='Date', ascending=False)


if 'row_index1' not in st.session_state:
    st.session_state.row_index1 = 0
if 'row_index2' not in st.session_state:
    st.session_state.row_index2 = 1

def display_rows(row_index1, row_index2):
    st.write("Row 1:")
    st.dataframe(spread_df.iloc[[row_index1]], column_order= st_order_col,width='stretch')
    st.write("Row 2:")
    st.dataframe(spread_df.iloc[[row_index2]], column_order= st_order_col,width='stretch')

#Create a line chart for the two selected rows
    sorted_df = spread_df.iloc[[row_index1, row_index2]]
    sorted_df = sorted_df[df_order_col].T
    fig = px.line(sorted_df)
    fig.data[0].line.color = 'darkgreen'
    fig.data[1].line.color = 'darkorange'
    for column in sorted_df.columns:
        for i, value in enumerate(sorted_df[column]):
            fig.add_annotation(x=sorted_df.index[i], y=value, text=str(value),
                            showarrow=False, font=dict(color='black'),yshift=10)
    # Add scatter plot with markers
    fig.add_trace(go.Scatter(
        x=sorted_df.index,
        y=sorted_df.iloc[:, 0],  # Taking values from the first selected row
        mode='markers',
        marker=dict(color='green'),
        showlegend=False  # Color for the first row's markers
    ))
    fig.add_trace(go.Scatter(
        x=sorted_df.index,
        y=sorted_df.iloc[:, 1],  # Taking values from the second selected row
        mode='markers',
        marker=dict(color='orange'),
        showlegend=False,  # Color for the second row's markers
    ))
    fig.update_layout(
        title=f"Interest Rates for {spread_df.index[row_index1]} and {spread_df.index[row_index2]}",
        xaxis_title="Maturity",
        yaxis_title="Interest Rate (%)",
        yaxis_range=[min(sorted_df.min()) * 0.9, max(sorted_df.max()) * 1.1],
        xaxis=dict(linecolor='black', linewidth=1), 
        yaxis=dict(linecolor='black', linewidth=1,dtick=0.25),
        autosize=True,
    )
    st.plotly_chart(fig) #,config={"scrollZoom": True}

# Button to go to the previous row for the first index

col1,col2 = st.columns(2)
with col1:
    if st.button('Index 1: Next Week'):
        if st.session_state.row_index1 > 0:
            st.session_state.row_index1 -= 1

    # Button to go to the next row for the first index
    if st.button('Index 1: Previous week'):
        if st.session_state.row_index1 < len(spread_df) - 1:
            st.session_state.row_index1 += 1
with col2:
    if st.button('Index 2: Next Week'):
        if st.session_state.row_index2 > 0:
            st.session_state.row_index2 -= 1

    # Button to go to the next row for the second index
    if st.button('Index 2: Previous week'):
        if st.session_state.row_index2 < len((spread_df)) - 1:
            st.session_state.row_index2 += 1

    if st.session_state.row_index2 <= st.session_state.row_index1:
        st.session_state.row_index2 = st.session_state.row_index1 + 1

max_spread = max(0, len(spread_df) - 1)
if st.session_state.row_index1 > max_spread:
    st.session_state.row_index1 = 0
if st.session_state.row_index2 > max_spread:
    st.session_state.row_index2 = min(1, max_spread)
if st.session_state.row_index2 <= st.session_state.row_index1:
    st.session_state.row_index2 = min(st.session_state.row_index1 + 1, max_spread)

if len(spread_df) >= 2:
    display_rows(st.session_state.row_index1,st.session_state.row_index2)

st.divider()
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
if 'row_index1' not in st.session_state:
    st.session_state.row_index1 = 0
if 'row_index2' not in st.session_state:
    st.session_state.row_index2 = 1

def display_rows(row_index1, row_index2):
    st.write("Row 1:")
    st.dataframe(tip_df.iloc[[row_index1]], column_order= st_order_col,width='stretch')
    st.write("Row 2:")
    st.dataframe(tip_df.iloc[[row_index2]], column_order= st_order_col,width='stretch')

#Create a line chart for the two selected rows
    sorted_df = tip_df.iloc[[row_index1, row_index2]]
    sorted_df = sorted_df[tip_order_col].T
    fig = px.line(sorted_df)
    fig.data[0].line.color = 'darkgreen'
    fig.data[1].line.color = 'darkorange'
    for column in sorted_df.columns:
        for i, value in enumerate(sorted_df[column]):
            fig.add_annotation(x=sorted_df.index[i], y=value, text=str(value),
                            showarrow=False, font=dict(color='black'),yshift=10)
    # Add scatter plot with markers
    fig.add_trace(go.Scatter(
        x=sorted_df.index,
        y=sorted_df.iloc[:, 0],  # Taking values from the first selected row
        mode='markers',
        marker=dict(color='green'),
        showlegend=False  # Color for the first row's markers
    ))
    fig.add_trace(go.Scatter(
        x=sorted_df.index,
        y=sorted_df.iloc[:, 1],  # Taking values from the second selected row
        mode='markers',
        marker=dict(color='orange'),
        showlegend=False,  # Color for the second row's markers
    ))
    fig.update_layout(
        title=f"Interest Rates for {tip_df.index[row_index1]} and {tip_df.index[row_index2]}",
        xaxis_title="Maturity",
        yaxis_title="Interest Rate (%)",
        yaxis_range=[min(sorted_df.min()) * 0.9, max(sorted_df.max()) * 1.1],
        xaxis=dict(linecolor='black', linewidth=1), 
        yaxis=dict(linecolor='black', linewidth=1,dtick=0.1),
        autosize=True,
    )
    st.plotly_chart(fig) #,config={"scrollZoom": True}

# Button to go to the previous row for the first index

col1,col2 = st.columns(2)
with col1:
    if st.button('Index 1: Next Week', key='button1'):
        if st.session_state.row_index1 > 0:
            st.session_state.row_index1 -= 1

    # Button to go to the next row for the first index
    if st.button('Index 1: Previous week',key='button2'):
        if st.session_state.row_index1 < len(tip_df) - 1:
            st.session_state.row_index1 += 1
with col2:
    if st.button('Index 2: Next Week',key='button3'):
        if st.session_state.row_index2 > 0:
            st.session_state.row_index2 -= 1

    # Button to go to the next row for the second index
    if st.button('Index 2: Previous week',key='button4'):
        if st.session_state.row_index2 < len((tip_df)) - 1:
            st.session_state.row_index2 += 1

    if st.session_state.row_index2 <= st.session_state.row_index1:
        st.session_state.row_index2 = st.session_state.row_index1 + 1

max_tip = max(0, len(tip_df) - 1)
if st.session_state.row_index1 > max_tip:
    st.session_state.row_index1 = 0
if st.session_state.row_index2 > max_tip:
    st.session_state.row_index2 = min(1, max_tip)
if st.session_state.row_index2 <= st.session_state.row_index1:
    st.session_state.row_index2 = min(st.session_state.row_index1 + 1, max_tip)

if len(tip_df) >= 2:
    display_rows(st.session_state.row_index1,st.session_state.row_index2)
st.dataframe(tip_df,column_order= st_order_col, width='stretch')
st.dataframe(spread_df,width='stretch')

st.divider()
#-----------------------------------------------------------------------------------------------------------------------------------------------------------
# Yield Curve Spread
