...
import pandas as pd
from dataloader import corp_oas_data
import plotly.express as px
import numpy as np


corp_oas, corp_etf_daily, corp_etf_weekly, corp_etf_monthly = corp_oas_data()
...
if freq == 'Daily':
    table = corp_etf_daily
elif freq == 'Weekly':
    table = corp_etf_weekly
else:
    table = corp_etf_monthly
...
display_summary_stats(df_diff, key_suffix='etf_stats')

#----------------------------------------------------------------------------------------------------------------------------------------------------
st.divider()

st.header("ICE BofA OAS Index")

oas_ratings = corp_oas['Rating'].dropna().unique()
oas_type = st.radio("OAS Rating", oas_ratings, horizontal=True)

oas_df = corp_oas[corp_oas['Rating'] == oas_type].copy()
oas_df = oas_df.sort_values('Date')
oas_df['Spread_Diff'] = oas_df['Values'].diff()

fig_oas1 = px.line(oas_df, x='Date', y='Values', title=f"{oas_type} Spread")
fig_oas1.update_layout(xaxis=dict(linecolor='black', linewidth=1, autorange=True, showgrid=True),
                       yaxis=dict(linecolor='black', linewidth=2), autosize=True)

fig_oas2 = px.bar(oas_df, x='Date', y='Spread_Diff', title=f"{oas_type} Spread Change")
fig_oas2.update_layout(xaxis=dict(linecolor='black', linewidth=1, autorange=True, showgrid=True),
                       yaxis=dict(linecolor='black', linewidth=2), autosize=True)

st.plotly_chart(fig_oas1)
st.plotly_chart(fig_oas2)

# Summary Stats
st.divider()
oas_diff = corp_oas[['Date', 'Rating', 'Values']].copy()
oas_diff = oas_diff.dropna(subset=['Rating'])
oas_diff = oas_diff.pivot_table(index='Date', columns='Rating', values='Values').reset_index()
for col in oas_diff.columns:
    if col != 'Date':
        oas_diff[col] = oas_diff[col].diff()

display_summary_stats(oas_diff, key_suffix='oas_stats')
