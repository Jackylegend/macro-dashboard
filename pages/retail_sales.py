...
from dataloader import retail_sales_data, line_chart

retail = retail_sales_data()
name = retail['Name'].unique()

index_df = retail.pivot_table(index='Date', columns='Name', values='Values').reset_index()
...
pct_mom['Values'] = pct_mom['Values'] * 100
pct_mom = pct_mom.sort_values(by='Values', ascending=True)
pct_yoy['Values'] = pct_yoy['Values'] * 100
...
fig1.update_layout(xaxis=dict(linecolor='black', tickfont=dict(color='black'), title='', linewidth=1, autorange=True, showgrid=True, nticks=10, tickformat=".2f%"),
                   yaxis=dict(linecolor='black', tickfont=dict(color='black'), title='', linewidth=2), autosize=True, height=600)
...
st.divider()
# ---------------------------------------------------------------
selected_values = st.multiselect('Select Index:', name, default=name[0])
table = retail[retail['Name'].isin(selected_values)]
table = table.pivot_table(index='Date', columns='Name', values='Values')

mom = table.copy().round(2)
yoy = table.copy().round(2)
...
with tab3:
    st.plotly_chart(line_chart(table), use_container_width=True)
    st.plotly_chart(line_chart(mom), use_container_width=True)
    st.plotly_chart(line_chart(yoy), use_container_width=True)
with tab4:
    st.dataframe(mom)
    st.dataframe(yoy)
...
stats_table = pd.concat([mean, stand_error, median, mode, std, s_var, kurt, skew, range_, min_, max_, sum_, count_], axis=1)
stats_table = stats_table.T.reset_index().rename(columns={'index': 'Descriptive Statistics'})
st.dataframe(stats_table)

st.divider()

value_columns = [col for col in stats_table.columns if col != 'Descriptive Statistics']
std_bounce = [{'Standard Deviation Bounds': '1 STD Lower Bound'}, {'Standard Deviation Bounds': '1 STD Upper Bound'},
              {'Standard Deviation Bounds': '2 STD Lower Bound'}, {'Standard Deviation Bounds': '2 STD Upper Bound'},
              {'Standard Deviation Bounds': '3 STD Lower Bound'}, {'Standard Deviation Bounds': '3 STD Upper Bound'},
              {'Standard Deviation Bounds': 'Actual Count'}, {'Standard Deviation Bounds': 'Actual Count'}, {'Standard Deviation Bounds': 'Actual Count'},
              {'Standard Deviation Bounds': 'Actual % Count'}, {'Standard Deviation Bounds': 'Actual % Count'}, {'Standard Deviation Bounds': 'Actual % Count'},
              {'Standard Deviation Bounds': 'Normal % Count'}, {'Standard Deviation Bounds': 'Normal % Count'}, {'Standard Deviation Bounds': 'Normal % Count'}]
posi_prob_adj = [{'Positive Probability Adjusted Return': 'Mean Change'}, {'Positive Probability Adjusted Return': 'Count'},
                 {'Positive Probability Adjusted Return': 'Frequency %'}, {'Positive Probability Adjusted Return': 'Prob Adj Change'}]
nega_prob_adj = [{'Negative Probability Adjusted Return': 'Mean Change'}, {'Negative Probability Adjusted Return': 'Count'},
                 {'Negative Probability Adjusted Return': 'Frequency %'}, {'Negative Probability Adjusted Return': 'Prob Adj Change'}]
zero_prob_adj = [{'Zero Return': 'Mean Change'}, {'Zero Return': 'Count'},
                 {'Zero Return': 'Frequency %'}, {'Zero Return': 'Prob Adj Change'}]
...
for col in value_columns:
    mean_row = stats_table.loc[stats_table['Descriptive Statistics'] == 'Mean', col].values
    std_row = stats_table.loc[stats_table['Descriptive Statistics'] == 'Standard Deviation', col].values
    total_count = stats_table.loc[stats_table['Descriptive Statistics'] == 'Count', col].values
...
for col in value_columns:
    percentiles_df[col] = np.nanpercentile(df_diff[col], percentile_values)
...
if selection:
    ...
    try:
        ...
        value_counts = vc.reindex(range(24), fill_value=0)
    except ValueError:
        value_counts = pd.Series(0, index=range(24))
    ...
    fig = px.bar(chart_table, x='Bin Label', y='Prob_num', title=f'{selection} Change Histogram',
