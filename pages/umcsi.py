...
import openpyxl
import pandas as pd
from dataloader import umcsi_data
import plotly.express as px
from datetime import timedelta
import numpy as np

umcsi = umcsi_data()
umcsi = umcsi.pivot_table(index='Date', columns='Index', values='Values').reset_index()
major_index = umcsi[['Date','Consumer Sentiment Index','Current Index','Expected Index','Inflation Expectation Index']]
comp_index = umcsi[['Date','Personal Finance Current','Personal Finance Expected','Business Condition 12 Months','Buying Conditions']]

option1 = st.multiselect("University of Michigan Consumer Sentiment Index:",list(major_index.columns.drop('Date')), default=list(major_index.columns.drop('Date'))[0])
major_index['Date'] = pd.to_datetime(major_index['Date'])
start_date = major_index['Date'].min()
end_date = major_index['Date'].max()
date_range = (end_date - start_date).days
extended_end_date = end_date + timedelta(days=int(date_range * 0.1))

major_index_pct_df = major_index.copy()
for col in option1:
    major_index_pct_df[col] = major_index[col].pct_change()
else:
    st.write("Please select at least one variable to display the chart.")

fig1 = px.line(major_index, x='Date', y=option1)
fig1.update_layout(xaxis=dict(linecolor='black', linewidth=1,showgrid=True,dtick='M12',tickangle=-45,tickmode='auto',
                                ),                   
                    yaxis=dict(linecolor='black', linewidth=2),autosize=True,)

fig2 = px.bar(major_index_pct_df, x='Date', y=option1,barmode='group')
fig2.update_layout(xaxis=dict(linecolor='black', linewidth=1,autorange=True,showgrid=True,tickangle=-45),
                    yaxis=dict(linecolor='black', linewidth=2),autosize=True,)
...
option2 = st.multiselect("Sub-Component Index:",list(comp_index.columns.drop('Date')),default=list(comp_index.columns.drop('Date'))[0])
comp_index_pct_df = comp_index.copy()
for col in option2:
    comp_index_pct_df[col] = comp_index[col].pct_change()
else:
    st.write("Please select at least one variable to display the chart.")

fig3 = px.line(comp_index, x='Date', y=option2)
fig3.update_layout(xaxis=dict(linecolor='black', linewidth=1,autorange=True,showgrid=True),
                    yaxis=dict(linecolor='black', linewidth=2),autosize=True,)

fig4 = px.bar(comp_index_pct_df, x='Date', y=option2,barmode='group')
fig4.update_layout(xaxis=dict(linecolor='black', linewidth=1,autorange=True,showgrid=True),
                    yaxis=dict(linecolor='black', linewidth=2),autosize=True,)
...
st.dataframe(dis_table,use_container_width=True)
st.plotly_chart(fig)
