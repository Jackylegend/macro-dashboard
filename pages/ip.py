...
from dataloader import ip_data, line_chart

# ── Data ────────────────────────────────────────────────────────────────────
ip = ip_data()

# ── Section 1: Category + Level selectors ───────────────────────────────────
col1, col2 = st.columns(2)
with col1:
    category = st.radio(
        "Select Industrial Production Category:",
        ['Market Group', 'Industry Group'],
        horizontal=True
    )

category_df = ip[ip['Category'] == category]

# Depth-1 nodes only (e.g. '1', '2', '3')
depth1_levels = sorted(
    [lvl for lvl in category_df['Index Level'].unique() if '.' not in lvl],
    key=lambda x: int(x)
)
depth1_names = {lvl: category_df[category_df['Index Level'] == lvl]['Name'].iloc[0] for lvl in depth1_levels}
...
# Depth-2 nodes that are direct children of the selected depth-1
depth2_df = category_df[category_df['Parent Index'] == top_level]
depth2_levels = sorted(depth2_df['Index Level'].unique(), key=lambda x: [int(p) for p in x.split('.')])
depth2_names = {lvl: category_df[category_df['Index Level'] == lvl]['Name'].iloc[0]
                for lvl in depth2_levels if not category_df[category_df['Index Level'] == lvl].empty}
...
# All series under selected top_level for the snapshot bar chart
level_df = category_df[
    category_df['Index Level'].str.startswith(top_level + '.') |
    (category_df['Index Level'] == top_level)
][['Date', 'Name', 'Values']]
...
# ── Section 2: Sub-level time series ────────────────────────────────────────
st.write("**Select series to plot over time**")

# Direct children of the selected sub-group
series_df = category_df[category_df['Parent Index'] == sub_group]
if series_df.empty:
    # sub_group is itself a leaf series
    series_df = category_df[category_df['Index Level'] == sub_group]

all_names = sorted(series_df['Name'].unique().tolist())

# Use multiselect for many series, checkboxes for few
if len(all_names) > 6:
...
if not selected:
    ...
    ip_mom = sel_pivot.copy().round(2)
    ip_yoy = sel_pivot.copy().round(2)
    for col in sel_pivot.columns:
        ip_mom[col] = sel_pivot[col].pct_change().round(4) * 100
        ip_yoy[col] = sel_pivot[col].pct_change(periods=12).round(4) * 100

    tab3, tab4 = st.tabs(["📈 Chart", "🗃 Data"])
    with tab3:
        st.caption("Level (Index Value)")
        st.plotly_chart(line_chart(sel_pivot), use_container_width=True, key='ts_level')
        st.caption("MoM % Change")
        st.plotly_chart(line_chart(ip_mom), use_container_width=True, key='ts_mom')
        st.caption("YoY % Change")
        st.plotly_chart(line_chart(ip_yoy), use_container_width=True, key='ts_yoy')
    with tab4:
        st.dataframe(sel_pivot.reset_index(), use_container_width=True)
        st.dataframe(ip_mom.reset_index(), use_container_width=True)
        st.dataframe(ip_yoy.reset_index(), use_container_width=True)

st.divider()

# ── Section 3: Descriptive Statistics ───────────────────────────────────────
# Based on the top-level index pivot (all series in this level group)
...
mode_res   = df_diff.mode(numeric_only=True)
mode_      = (mode_res.iloc[0] if not mode_res.empty else df_diff.mean(numeric_only=True)).to_frame("Mode")
...
for col in value_columns:
    ...
    posi_count = int(np.sum(vals > 0));  posi_freq = posi_count / total_n
    posi_prob_adj[0][col] = round(float(np.mean(vals[vals > 0])) if posi_count else 0, 4)
    posi_prob_adj[1][col] = posi_count
    posi_prob_adj[2][col] = f"{posi_freq:.2%}"
    posi_prob_adj[3][col] = round(posi_prob_adj[0][col] * posi_freq, 4)

    nega_count = int(np.sum(vals < 0));  nega_freq = nega_count / total_n
    nega_prob_adj[0][col] = round(float(np.mean(vals[vals < 0])) if nega_count else 0, 4)
    nega_prob_adj[1][col] = nega_count
    nega_prob_adj[2][col] = f"{nega_freq:.2%}"
    nega_prob_adj[3][col] = round(nega_prob_adj[0][col] * nega_freq, 4)

    zero_count = int(np.sum(vals == 0)); zero_freq = zero_count / total_n
    zero_prob_adj[0][col] = round(float(np.mean(vals[vals == 0])) if zero_count else 0, 4)
...
dist_cols = [c for c in df_diff.columns if c != 'Date']
